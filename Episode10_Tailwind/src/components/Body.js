  ...
  return filteredListOfRes?.length === 0  ? (
    ...
    <div className="body p-8">
      <div className="filters flex ">
        ...
        <button
          className={`filter px-5 py-1 shadow ease-in transition-all rounded ${isTopRatedOpen ? " bg-green-500 text-white" : "bg-slate-200"}`}
          onClick={() => {
      ...
      <div className="res-container flex justify-center gap-8 flex-wrap"  style={{ margin: "50px 0" }}>
        {filteredListOfRes.map((resData) => {
          return <Link to={`/restaurants/${resData.info.id}`} className="block w-72" key={resData.info.id}><RestaurantCard resData={resData}  /></Link>;
        })}
      </div>
    </div>
  );
